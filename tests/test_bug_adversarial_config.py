...
def test_adversarial_mode_config_loading(tmp_path):
    ...
    config_data = {
        "council": {"adversarial_mode": True, "models": ["model1", "model2", "model3", "model4"]}
    }
    ...
    assert config.council.adversarial_mode is True, (
        f"Expected adversarial_mode to be True from YAML, got {config.council.adversarial_mode}"
    )
